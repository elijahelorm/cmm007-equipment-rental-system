...
router.post("/equipment/add", async (req, res) => {
  try {
    const { name, category, serial_number, condition_status, total_quantity } =
      req.body;
    ...
    if (existing.length > 0) {
      return res.redirect(
        "/admin/equipment?error=Serial number already exists",
      );
    }
...
router.get("/equipment/delete/:id", async (req, res) => {
  try {
    ...
    if (activeRentals.length > 0) {
      return res.redirect(
        "/admin/equipment?error=Cannot delete - equipment is currently rented",
      );
    }

    // Delete equipment
    await pool.query("DELETE FROM equipment WHERE equipment_id = ?", [
      req.params.id,
    ]);
...
router.get("/users/delete/:id", async (req, res) => {
  try {
    ...
    if (activeRentals.length > 0) {
      return res.redirect(
        "/admin/users?error=Cannot delete - user has active rentals",
      );
    }
...
router.get("/settings", async (req, res) => {
  try {
    const [limits] = await pool.query("SELECT * FROM rental_limits");
...
// Update rental limits
router.post("/settings/limits", async (req, res) => {
  try {
    const { user_limit, admin_limit } = req.body;

    await pool.query(
      'UPDATE rental_limits SET max_items = ? WHERE role = "user"',
      [user_limit],
    );
    await pool.query(
      'UPDATE rental_limits SET max_items = ? WHERE role = "admin"',
      [admin_limit],
    );

    res.redirect("/admin/settings?success=Rental limits updated successfully");
  } catch (error) {
    console.error("Update limits error:", error);
    res.redirect("/admin/settings?error=Failed to update limits");
  }
});
